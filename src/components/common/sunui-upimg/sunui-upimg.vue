	...
	async created() {
		...
		setTimeout(() => {
			...
			this.upimg_preview.map(item => {
				// step2.这里修改服务器返回字段 ！！！
				console.log(item)
				this.upload_cache_list.push(item.id);
			});
			this.emit();
		...
		chooseImage() {
			...
			uni.chooseImage({
				...
				success: function(res) {
					let file =  res.tempFiles[res.tempFiles.length - 1]
					// const isRightType = file.name.indexOf('.png') > -1
					const isRightSize = file.size / 1024 / 1024 < 10;
					console.log(isRightSize)
					if(isRightSize) {
						for (let i = 0, len = res.tempFiles.length; i < len; i++) {
							res.tempFiles[i]['upload_percent'] = 0;
							_self.upload_before_list.push(res.tempFiles[i]);
						}
						_self.uid = file.lastModified
...
const upload = function(options) {
	let url = options.url,
		...
		// 	extra.token = service.getUsers()[0].token
		// }
		// extra.businessId = _self.userId
	const uploadTask = uni.uploadFile({
		...
		success: function(res) {
			...
			if (res.statusCode == 200) {
				let pasRES = JSON.parse(res.data)
				console.log(pasRES)
					if(pasRES.success){
						uni.showToast({
							icon:'none',
						...
						_self.upload_cache_list.push(pasRES.id)
						// if (success) {
						// 	success(data);
						// }
						// _this.$emit('success',pasRES.)
						_self.emit();
